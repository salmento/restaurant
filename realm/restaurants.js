...
exports = async function(payload, response) {
  ...
  const { restaurantsPerPage = 21, page = 0 }  = payload.query;
  ...
  let restaurantsList = await collection.find(query).skip( restaurantsPerPage*page).limit( restaurantsPerPage ).toArray();
